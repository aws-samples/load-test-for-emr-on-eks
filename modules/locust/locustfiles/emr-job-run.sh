...
export S3BUCKET="emr-on-${CLUSTER_NAME}-${ACCOUNTID}"
export ECR_URL="${ACCOUNTID}.dkr.ecr.${AWS_REGION}.amazonaws.com"
export EMR_VERSION="emr-${EMR_IMAGE_VERSION:-"7.9.0"}-latest"
export SELECTED_AZ=${SELECTED_AZ}
export KMS_ARN=$(aws kms describe-key --key-id arn:aws:kms:${AWS_REGION}:${ACCOUNTID}:alias/emr-eks-load-test --query 'KeyMetadata.Arn' --output text)
