...
# Table of Contents
- [Automate with the MCP server (quick start)](#automate-with-the-mcp-server-quick-start)
...
## Automate with the MCP server (quick start)

This repo ships an **MCP server** ([`loadtest_mcp/`](./loadtest_mcp/)) that automates the whole workflow below — confirming your AWS profile, choosing to reuse or create an EKS cluster, provisioning, running tests, monitoring, and cleanup — by wrapping the same scripts documented in this README. Point any MCP client (Claude Code, Kiro, etc.) at it and drive the load test conversationally. Full tool reference: [the MCP server README](./README.md) (repo root).

### 1. Install the server

The server needs the same CLIs this repo uses (`aws`, `kubectl`, `eksctl`, `helm`, `git`) plus the `mcp[cli]` Python package (Python ≥ 3.10). It clones the load-test artifacts from GitHub on first use, so no manual checkout is required.

**Option A — AIM (Amazon AI Integration Manager), recommended for internal users.** Bootstraps everything from GitHub:
```bash
mwinit
...
The load-test server runs the AWS CLI itself, so it works standalone. But pairing it with the **AWS MCP server** (`aws-mcp`, which exposes `call_aws`, `search_documentation`, `list_regions`, etc.) lets the assistant verify regions, look up EMR/EKS docs, and run AWS API calls directly — handy for the reuse-vs-create decision and troubleshooting.

```bash
# Via AIM:
aim mcp install aws-mcp

# Or via Claude Code (plugin marketplace):
claude plugin marketplace add aws/agent-toolkit-for-aws
claude plugin install aws-core@agent-toolkit-for-aws   # provides the aws-mcp server
```

### 3. Start using it

The account and region are derived from your **active AWS profile** (nothing is hardcoded), so confirm that first, then drive the flow:

1. `get_aws_profile` — confirm the account/region are correct (use `set_aws_profile <name>` to switch).
2. `get_env` — review the resolved load-test settings.
3. `check_eks_cluster` — does the target EKS cluster exist?
   - **Reuse:** `use_existing_eks_cluster <name>` to point the test at it.
   - **Create:** `prepare_new_eks_cluster --eks_version 1.35 --cluster_name <name>` then `provision_infra` (background, ~20–40 min).
4. `provision_locust_operator` → `refresh_configmap` → `apply_eks_test` to run a distributed test (or `run_local_test` for a quick smoke test).
5. Monitor with `get_test_logs`, `list_virtual_clusters`, `get_job_runs`, `get_grafana_login`.
6. Clean up with `stop_test`, `delete_test_namespaces`, and `teardown_infra`.

Prefer to run everything by hand instead? The rest of this README walks through the same steps manually.
